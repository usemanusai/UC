...
UC features an **Automated Proxy Health Checker & Fetcher** (via `ProxyHealthDaemon` and `ProxySourceWorker`). When enabled:
- The UI accepts a `Proxy Source URL` and a configurable `Fetch Interval`.
- A background daemon (`ProxySourceWorker` thread) automatically and periodically fetches fresh proxies from configurable HTTP source URLs while validation is active.
- An active TCP health checker (`ProxyHealthDaemon`) runs in the background, testing connections to loaded proxies (via `socket.connect_ex`) and reporting successes or failures directly to the rotator.
